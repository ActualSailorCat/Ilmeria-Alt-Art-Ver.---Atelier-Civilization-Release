...
VALUES	
-- YIELDS
		('SAILOR_ILMERIA_ALT_Yields',			1,			'YIELD_FOOD',						0),		
		('SAILOR_ILMERIA_ALT_Yields',			1,			'YIELD_SCIENCE',					0),		
-- PSEUDOYIELDS
		('SAILOR_ILMERIA_ALT_PseudoYields',		1,			'PSEUDOYIELD_GPP_SCIENTIST',		25),
...
-- BUILDINGS & DISTRICTS
		...
		('SAILOR_ILMERIA_ALT_Districts',		1,			'DISTRICT_CAMPUS',					25),	
...
FROM Units WHERE EXISTS (SELECT 1 FROM Units WHERE UnitType = 'UNIT_SKIRMISHER');
-- BUILDINGS
INSERT INTO AiFavoredItems	
		(ListType,							Favored,	Item,				Value)
...
FROM Buildings WHERE EXISTS (SELECT 1 FROM Buildings WHERE BuildingType = 'BUILDING_HALICARNASSUS_MAUSOLEUM');